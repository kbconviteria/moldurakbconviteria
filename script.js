...
// ===== CONFIG por cliente (sem arquivo extra) =====
// Usa "tela-inicial.png" e "moldura.png" por padrão.
// Permite sobrescrever via URL:
//   ?capa=img.png&moldura=frame.png&titulo=Oi&sub=Texto&botao=Abrir
(function aplicarConfigDinamica(){
  const q = new URLSearchParams(location.search);
  const capa   = q.get('capa')    || 'tela-inicial.png';
  const mold   = q.get('moldura') || 'moldura.png';
  const titulo = q.get('titulo')  || 'Seja bem vindo (a)!';
  const sub    = q.get('sub')     || 'A nossa moldura web.';
  const botao  = q.get('botao')   || 'Toque para abrir';

  // capa
  startScr.style.backgroundImage = `url("${capa}")`;
  // textos
  const t = document.getElementById('startTitle');
  const s = document.getElementById('startSubtitle');
  if (t) t.textContent = titulo;
  if (s) s.textContent = sub;
  if (botao) btnStart.setAttribute('aria-label', botao);
